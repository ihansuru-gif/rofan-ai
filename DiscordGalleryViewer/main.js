...
function sanitizeJitsiBase(raw) {
  ...
  if (!['https:', 'http:'].includes(url.protocol)) throw new Error('Jitsi 주소는 http/https만 사용할 수 있습니다.');
  url.pathname = '/';
  url.search = '';
  url.hash = '';
  return url.origin;
...
function notifyPiPState() {
  if (isUsable(mainWindow)) mainWindow.webContents.send('dgv:pip-state', isUsable(pipWindow));
}
...
function finishDisplaySelection(sourceId) {
  ...
  try {
    if (source) callback({ video: source, ...(audioRequested && process.platform === 'win32' ? { audio: 'loopback' } : {}) });
    else callback({});
  } catch (_) {}
...
function cancelDisplayRequest() {
  if (pendingDisplayRequest?.callback) {
    try { pendingDisplayRequest.callback({}); } catch (_) {}
  }
  pendingDisplayRequest = null;
  if (isUsable(sourcePickerWindow)) sourcePickerWindow.destroy();
  sourcePickerWindow = null;
...
async function openSourcePicker(request, callback) {
  ...
  pendingDisplayRequest = { callback, request, sources };
  ...
  sourcePickerWindow.webContents.on('will-navigate', (event, target) => {
    ...
    try {
      ...
      if (parsed.hostname === 'cancel') cancelDisplayRequest();
      else if (parsed.hostname === 'source') finishDisplaySelection(decodeURIComponent(parsed.pathname.replace(/^\//, '')));
    } catch (_) {
      cancelDisplayRequest();
    }
  });
  ...
  await sourcePickerWindow.loadURL(`data:text/html;charset=utf-8,${encodeURIComponent(sourcePickerHtml(sources))}`);
}
...
function configureSession(ses) {
  ...
  ses.setPermissionCheckHandler((webContents, permission, requestingOrigin) => {
    return Boolean(isTrustedProviderUrl(requestingOrigin || '') && ['media', 'fullscreen', 'notifications'].includes(permission));
  });
...
async function openProvider(provider, options = {}) {
  if (!isUsable(mainWindow)) return { ok: false, error: '창이 준비되지 않았습니다.' };
...
function createWindow() {
  ...
  mainWindow.webContents.setWindowOpenHandler(({ url }) => {
    if (isTrustedProviderUrl(url)) return { action: 'allow' };
...
ipcMain.handle('dgv:open-provider', (event, provider, options) => {
  if (!isUsable(mainWindow) || event.sender !== mainWindow.webContents) return { ok: false, error: '잘못된 요청입니다.' };
...
ipcMain.handle('dgv:get-provider', () => currentProvider);
